...
$create_table_sql = "create table Employees (supid char(4),comname char(40),address char(40),post char(40),phone char(40))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = $r->fetch_object()) {
    if (isset($_REQUEST{'id'}) && $_REQUEST{'id'}  == $o->supid) $chg = " style='background-color:#f9f9f9"; else $chg = " readonly style='background-color:#ffffdd";
    echo "<tr><form action='' method=post>
      <td><input name=id size=5 value='".	$o->supid . "' style='background-color:#dddddd' readonly></td>
      <td><input name=a size=40 value='". $o->comname . "' $chg'></td>
      <td><input name=b size=20 value='". $o->address . "' $chg;text-align:right'></td>
	  <td><input name=c size=20 value='". $o->post . "' $chg;text-align:right'></td>
	  <td><input name=d size=20 value='". $o->phone . "' $chg;text-align:right'></td>
      <td>";
    if (isset($_REQUEST{'id'}) && $_REQUEST{'id'} == $o->supid) {
...
if ($act == "del : confirm") {
  ...
  $q  = "delete from $tb where supid ='". $_REQUEST{'id'} . "'";
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
...
if ($act == "edit : confirm") {
  $q  = "update $tb set comname ='". $_REQUEST{'a'} . "', address ='". $_REQUEST{'b'} . "', post ='". $_REQUEST{'c'} . "', phone ='". $_REQUEST{'d'} . "' where supid =" . $_REQUEST{'id'};
  $conn = new mysqli($host, $user, $password, $db);
  $r = $conn->query($q);
...
function checkid($id,$msg){
	$sql = ("select * from suppliers where supid ='".$id."' ");
	...
	echo $obj->supid.": Can not $msg จะกลับไปหน้าเดิมใน 3วิ";
	exit;
}
}
?>
